...
import { ProjectBalanceDisplay } from '@/components/project-balance-display';
...
import { getProjects, correctInconsistentProjects } from '@/services/projectService';
import { getAllPayments } from '@/services/paymentService';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import { ProjectType } from '@/types/project';

// --- TIPOS Y COMPONENTES AUXILIARES (DEFINIDOS FUERA) ---
...
const StatCard: React.FC<StatCardProps> = ({ title, value, description, icon: Icon, loading, colorClass = 'text-primary', change }) => (
              ...
              leftContent={
                <div className='space-y-1'>
                  <div className='flex items-center'>
                    ...
                    {project.glosa && <span className='text-muted-foreground ml-1'>- {project.glosa}</span>}
                  </div>
                  <div className='text-xs text-muted-foreground ml-8'>
                    {new Date(project.date).toLocaleDateString()} - {formatCurrency(project.total)}
                  </div>
                </div>
              }
...
interface ProjectsToCollectListProps {
  projects: ProjectType[];
  loading: boolean;
}

const ProjectsToCollectList: React.FC<ProjectsToCollectListProps> = ({ projects, loading }) => {
  const projectsToShow = projects.filter(
    (project) => project.collect === true && project.isPaid === false && (project.balance === undefined || project.balance > 0)
  );
        ...
        {loading ? (
          ...
          <div className='space-y-2'>
            {projectsToShow.map((project) => (
              <DashboardProjectItem
                key={project.id}
                project={project}
                leftContent={<ProjectClientDisplay project={project} />}
                rightContent={<ProjectBalanceDisplay project={project} />}
              />
            ))}
          </div>
        ) : (
          <div className='text-center py-10'>
            ...
            <p className='text-sm text-muted-foreground'>
              {projects.length > 0 ? 'No hay proyectos pendientes por cobrar.' : 'No hay proyectos marcados para cobrar.'}
            </p>
          </div>
        )}
      </CardContent>
...
export default function DashboardPage() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      const recentPayments = paymentsData.filter((p: any) => new Date(p.date) >= thirtyDaysAgo);
      ...
      setRecentProjects(
        [...projects].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5)
      );
      ...
      setProjectsToCollect(allProjectsToCollect.slice(0, 5));
    } catch (error) {
  ...
  const handleCorrectProjects = async () => {
    setIsCorrecting(true);
    try {
      const { correctedCount } = await correctInconsistentProjects();
      alert(`${correctedCount} proyectos han sido corregidos.`);
      await loadDashboardData();
    } catch (error) {
      console.error('Error correcting projects:', error);
      alert('Hubo un error al corregir los proyectos.');
    } finally {
        ...
        {data.isLoading ? (
          ...
          <div className='space-y-6'>
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4'>
              <StatCard title='Proyectos Totales' value={data.totalProjects} description='Total de proyectos registrados' icon={Briefcase} loading={data.isLoading} change={{ value: 5, isPositive: true }} />
              <StatCard title='Proyectos Activos' value={data.activeProjects} description='Proyectos pendientes' icon={Clock} loading={data.isLoading} colorClass='text-amber-500' />
              <StatCard title='Clientes' value={data.totalClients} description='Total de clientes' icon={Users} loading={data.isLoading} />
              <StatCard title='Pagos Recibidos' value={formatCurrency(data.recentPaymentsAmount)} description='Pagos últimos 30 días' icon={DollarSign} loading={data.isLoading} colorClass='text-green-500' />
            </div>
            ...
            <div className='mb-4'>
              <Button onClick={handleCorrectProjects} disabled={isCorrecting || data.isLoading}>
                {isCorrecting ? 'Corrigiendo...' : 'Corregir Inconsistencias de Pago'}
              </Button>
            </div>
          </div>
        )}
